...
- `/v1/messages` is compat mode and is expected to emit `compat_provider_pinned`.
...
- `provider` in the proxy body selects the requested provider, but provider pinning is controlled by pin signal (`x-innies-provider-pin` / metadata) or compat mode.
...
| Compat requests (`/v1/messages`) | Requests emit `compat_provider_pinned` and stay on Anthropic | compat route events |
