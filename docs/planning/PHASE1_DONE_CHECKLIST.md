...
| 2 | Per-buyer-key provider preference | DONE | Deterministic preference-first routing with fallback on 401/403/429/timeout/5xx/capacity. `preferredProviderSource` distinguishes explicit vs default. Admin endpoints for GET/PUT preference. Shell scripts: `innies-set-preference.sh`, `innies-get-preference.sh`, `innies-check-preference.sh`. |
...
| 6 | Token onboarding | DONE | Admin endpoints: add (`POST /v1/admin/token-credentials`), rotate, revoke, pause/unpause, probe, provider-usage-refresh. Debug labels for attribution. Health checks + quarantine lifecycle (active/maxed/probe/revoked). Shell scripts: `innies-add-token.sh`, `innies-rotate-token.sh`, `innies-pause-token.sh`, plus 8 more operator scripts. |
...
| 2 | Provider preference | Admin preference endpoints exercised via `innies-set-preference.sh`, `innies-get-preference.sh`, `innies-check-preference.sh`. Fallback reason codes emitted in route decision metadata. Integration tests cover preference behavior, fallback, and re-entry. |
...
|------|---------|----------|
| Hidden dashboard columns | `Util 24h` and `Maxed 7d` columns are temporarily hidden in the analytics token table for layout clarity. Bring both columns back after the dashboard gets a cleaner slot for operator-only health fields. | Low — data is available via API, just not shown in UI. |
| Pending script rename | Operator scripts in `scripts/` use old naming convention (e.g., `innies-add-token.sh`). Design spec complete at `docs/superpowers/specs/2026-03-16-script-command-renaming-design.md` for rename to noun-first pattern (e.g., `innies-token-add`). Rename not yet executed. | Low — scripts work, just inconsistent naming. |
