-- =========================================================
-- Headroom C1 schema only (no extensions required)
-- PostgreSQL 15+
-- Notes:
-- - No pgcrypto: UUID defaults removed (app must provide IDs)
-- - No citext: email is text with lower(email) unique index
-- - No btree_gist: model window overlap enforced in app logic
-- =========================================================
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_hr_users_email_lower ON hr_users ((lower(email)));
...
-- NOTE: no exclusion constraint here (requires btree_gist).
-- Enforce non-overlapping windows in application logic for C1.
