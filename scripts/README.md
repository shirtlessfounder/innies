...
- `innies-compat-artifact-index` writes `summary.txt` plus `summary.json`, classifying each reconstructed request as `known_good_candidate`, `invalid_request_candidate`, or `other`
- `innies-compat-artifact-index` usage:
  - `INNIES_COMPAT_ARTIFACT_INDEX_OUT_DIR=/tmp/issue80-index scripts/innies-compat-artifact-index.sh /path/to/response.html anthropic 400`
