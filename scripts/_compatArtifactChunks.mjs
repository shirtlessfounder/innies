export const COMPAT_ARTIFACT_CHUNK_LABELS = new Set([
...
export function collectChunkBlocks(contents, allowedLabels = COMPAT_ARTIFACT_CHUNK_LABELS) {
...
function createSequence(block) {
  ...
  return {
    ...
    lastChunkIndex: block.chunkIndex,
    parts,
  };
}

function isSequenceComplete(sequence) {
  return Array.from({ length: sequence.chunkCount }, (_, index) => {
    return typeof sequence.parts[index] === 'string';
  }).every(Boolean);
}

function findCompatibleSequenceIndex(activeSequences, block) {
  if (block.chunkIndex === 0) {
    return -1;
  }

  return activeSequences.findIndex((sequence) => {
    return (
      sequence.chunkCount === block.chunkCount &&
      sequence.parts[block.chunkIndex] === undefined &&
      block.chunkIndex > sequence.lastChunkIndex
    );
  });
}
...
export function reconstructEntries(blocks) {
  ...
  for (const block of blocks) {
    const sequenceIndex = findCompatibleSequenceIndex(activeSequences, block);
    const targetIndex =
      sequenceIndex === -1
        ? activeSequences.push(createSequence(block)) - 1
        : sequenceIndex;

    if (sequenceIndex !== -1) {
      const sequence = activeSequences[targetIndex];
      sequence.parts[block.chunkIndex] = block.jsonChunk;
      sequence.lastChunkIndex = block.chunkIndex;
    }

    if (isSequenceComplete(activeSequences[targetIndex])) {
      completed.push(activeSequences[targetIndex].parts.join(''));
      activeSequences.splice(targetIndex, 1);
    }
  }
...
export function parseEntriesByLabel(chunkBlocks) {
  ...
  for (const [label, blocks] of chunkBlocks.entries()) {
    parsed.set(
      label,
      reconstructEntries(blocks).map((jsonText) => JSON.parse(jsonText)),
    );
  }
