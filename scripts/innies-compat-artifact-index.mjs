...
import {
  collectChunkBlocks,
  parseEntriesByLabel
} from './_compatArtifactChunkParser.mjs';
...
const chunkBlocks = collectChunkBlocks(artifactContents, CHUNK_LABELS);
const entriesByLabel = new Map(
  Array.from(parseEntriesByLabel(chunkBlocks).entries(), ([label, entries]) => {
    return [
      label,
      entries.filter((entry) => entry && typeof entry === 'object' && typeof entry.request_id === 'string')
    ];
  })
);
